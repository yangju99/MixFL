...
model = AutoModelForCausalLM.from_pretrained("facebook/incoder-1B", output_hidden_states=True)
model.eval()
...
def get_context_embedding(codebase: str, method: str = "avg", max_tokens: int = 2048) -> torch.Tensor:
    """
    Generate code embedding using InCoder (decoder-only causal model).

    Args:
        codebase (str): Input source code
        method (str): One of ['first', 'avg', 'last']
        max_tokens (int): Maximum tokens per chunk (<=2048 for incoder-1B)

    Returns:
        torch.Tensor: Embedding vector (shape: [hidden_dim])
    """
    tokens = tokenizer.tokenize(codebase)
    chunk_size = max_tokens - 1  # leave space for special tokens
    ...
    final_embedding = torch.stack(embeddings, dim=0).mean(dim=0).squeeze(0)  # [hidden_dim]
